...
export function EditTransactionForm({ 
  ...
  const [recurrence, setRecurrence] = useState<string>(transaction.recurrence || '');
  const [recurringMode, setRecurringMode] = useState<string>(transaction.recurring_mode || 'REMINDER_ONLY');
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (['TRANSFER', 'SAVING'].includes(currentType) && (!accountId || !toAccountId)) {
      toast.error(currentType === 'SAVING' ? 'উৎস ও সঞ্চয় অ্যাকাউন্ট উভয়ই নির্বাচন করুন' : 'Please select both source and destination accounts');
      setIsLoading(false);
      return;
    }
  ...
  const requiresSingleAccount = ['INCOME', 'EXPENSE', 'GIVEN', 'RECEIVED', 'BORROWED', 'RETURNED'].includes(currentType);
  const requiresTwoAccounts = ['TRANSFER', 'SAVING'].includes(currentType);
        ...
        <form onSubmit={handleSubmit}>
          ...
          {requiresTwoAccounts && (
            <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto_1fr] gap-3 items-start">
              <div className="space-y-2">
                <Label>{currentType === 'SAVING' ? (t.addTransactionForm.fromAccount || 'কোন অ্যাকাউন্ট থেকে (From)') : t.addTransactionForm.source}</Label>
                <Select value={accountId} onValueChange={(val) => setAccountId(val || '')} required>
                  <SelectTrigger className="w-full glass-panel border-primary/20">
                    <SelectValue placeholder={currentType === 'SAVING' ? (t.addTransactionForm.fromAccount || 'উৎস অ্যাকাউন্ট') : t.addTransactionForm.source}>
                      {accountId ? accounts.find(a => a.id === accountId)?.name : (currentType === 'SAVING' ? (t.addTransactionForm.fromAccount || 'উৎস অ্যাকাউন্ট') : t.addTransactionForm.source)}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectContent className="glass-panel border-primary/20 shadow-2xl">
                    {accounts.map(acc => (
              ...
              <div className="space-y-2">
                <Label>{currentType === 'SAVING' ? (t.addTransactionForm.toAccount || 'সঞ্চয় অ্যাকাউন্ট (To)') : t.addTransactionForm.destination}</Label>
                <Select value={toAccountId} onValueChange={(val) => setToAccountId(val || '')} required>
                  <SelectTrigger className="w-full glass-panel border-primary/20">
                    <SelectValue placeholder={currentType === 'SAVING' ? (t.addTransactionForm.toAccount || 'সঞ্চয় অ্যাকাউন্ট') : t.addTransactionForm.destination}>
                      {toAccountId ? accounts.find(a => a.id === toAccountId)?.name : (currentType === 'SAVING' ? (t.addTransactionForm.toAccount || 'সঞ্চয় অ্যাকাউন্ট') : t.addTransactionForm.destination)}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectContent className="glass-panel border-primary/20 shadow-2xl">
                    {accounts.map(acc => (
          ...
          {/* RECURRING OPTIONS */}
          {['INCOME', 'EXPENSE', 'SAVING', 'TRANSFER', 'RETURNED'].includes(currentType) && (
